 ...
 * @twitter:   @cnttaddy
 * @github :   https://github.com/niitettehtsuru/HarmonicBop
 * @codepen:   https://codepen.io/niitettehtsuru/pen/VweBZpe
...
class Lissajous
{
    ...
    {    
        this.drawCircularRing();//draw ring around the wave
        this.drawWave();//draw wave
    }  
    drawCircularRing()//draws the circular/elliptical ring around the wave
    {    
        ...
        {  
            ...
            let x = (this.relativeWidth  * Math.sin(this.numHorizontalTangents*this.parameter + this.rotationAngle))+this.xCoord + Math.tan(this.xPrev);//first equation  
            let y = (this.relativeHeight * Math.sin(this.numVerticalTangents*this.parameter))+this.yCoord - Math.tan(this.xPrev);//second equation  
            if(i > 0)
            {
    ...
    drawWave()//draws the wavelike thingy
    {    
        this.parameter = 0;//reset parameter 
        strokeWeight(0.7);
        stroke(this.color);  
